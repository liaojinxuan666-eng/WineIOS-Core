...
#define WIOS_MAIN_PROBE_STAGE_DEBUG_INIT 7u
...
typedef const char *(*wios_ntdll_get_first_teb_probe_string_fn)(void);
...
static int probe_wine_main_entry(const wios_runtime_config *config)
{
    ...
    wios_ntdll_get_first_teb_probe_string_fn get_first_teb_fixed_map_probe;
    ...
    ntdll_set_main_probe_stage(WIOS_MAIN_PROBE_STAGE_DEBUG_INIT);
    ...
    first_teb_fixed_map_probe = get_first_teb_fixed_map_probe();
    runtime_log(config, first_teb_fixed_map_probe && first_teb_fixed_map_probe[0]
                         ? first_teb_fixed_map_probe
                         : "WINE_FIRST_TEB_FIXED_MAP=EMPTY");
    ...
    if (get_main_probe_result_stage() != WIOS_MAIN_PROBE_STAGE_DEBUG_INIT)
    {
        set_error("Wine debug-init probe returned before dbg_init completed");
        runtime_log(config, "WINE_FIRST_TEB_SETUP=PASS_HOST_PROBE_ONLY");
        runtime_log(config, "WINE_SIGNAL_INIT_THREADING=PASS");
        runtime_log(config, "WINE_DEBUG_INIT=INCOMPLETE");
        runtime_log(config, "WINE_MAIN_THREAD_INIT=FAIL");
        return -13;
    }
    runtime_log(config, "WINE_FIRST_TEB_SETUP=PASS_HOST_PROBE_ONLY");
    ...
    runtime_log(config, "WINE_MAIN_STOP_AFTER=DBG_INIT");
