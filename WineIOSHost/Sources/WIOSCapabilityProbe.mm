...
static void WIOSProbeSharedUserDataAddress(void)
{
    ...
    if (kr == KERN_SUCCESS) {
        ...
        targetAlreadyMapped = containsTarget;
    ...
    /*
     * The read-only probe above proved whether the target is occupied. If it is
     * a hole, perform one tightly scoped Darwin fixed-address allocation test
     * using the same primitive Wine can use, then immediately release it.
     * This does not leave 0x7ffe0000 mapped for the Wine runtime.
     */
    long hostPageLong = sysconf(_SC_PAGESIZE);
    if (targetAlreadyMapped) {
        [log appendLevel:@"WARN"
                      key:@"MACH_VM_FIXED_7FFE0000"
                    value:@"SKIPPED_TARGET_ALREADY_MAPPED"];
        [log appendLevel:@"WARN"
                      key:@"MACH_VM_7FFE0000_MUTATION"
                    value:@"SKIPPED_TARGET_ALREADY_MAPPED"];
    } else if (hostPageLong <= 0 ||
               ((mach_vm_address_t)target & ((mach_vm_address_t)hostPageLong - 1)) != 0) {
        [log appendLevel:@"WARN"
                      key:@"MACH_VM_FIXED_7FFE0000"
                    value:[NSString stringWithFormat:
                        @"SKIPPED_INVALID_PAGE_SIZE page=%ld", hostPageLong]];
        [log appendLevel:@"WARN"
                      key:@"MACH_VM_7FFE0000_MUTATION"
                    value:@"SKIPPED_INVALID_PAGE_SIZE"];
    } else {
        mach_vm_address_t fixedAddress = (mach_vm_address_t)target;
        mach_vm_size_t fixedSize = (mach_vm_size_t)hostPageLong;

        [log appendLevel:@"INFO"
                      key:@"MACH_VM_FIXED_7FFE0000"
                    value:[NSString stringWithFormat:
                        @"BEGIN requested=0x%016llX size=0x%llX",
                        (unsigned long long)fixedAddress,
                        (unsigned long long)fixedSize]];

        kern_return_t mapKr = mach_vm_map(
            mach_task_self(),
            &fixedAddress,
            fixedSize,
            0,
        ...
        kern_return_t releaseKr = KERN_FAILURE;
        if (mapKr == KERN_SUCCESS) {
            releaseKr = mach_vm_deallocate(
                mach_task_self(), fixedAddress, fixedSize);
        }

        [log appendLevel:(mapKr == KERN_SUCCESS && releaseKr == KERN_SUCCESS)
                           ? @"INFO" : @"WARN"
                      key:@"MACH_VM_FIXED_7FFE0000"
                    value:[NSString stringWithFormat:
                        @"map_kr=%d returned=0x%016llX size=0x%llX release_kr=%d",
                        mapKr,
                        (unsigned long long)fixedAddress,
                        (unsigned long long)fixedSize,
                        releaseKr]];
        ...
        if (mapKr != KERN_SUCCESS) {
            [log appendLevel:@"INFO"
                          key:@"MACH_VM_7FFE0000_MUTATION"
                        value:@"FIXED_MAP_ATTEMPT_FAILED_NO_MAPPING_CREATED"];
        } else if (releaseKr == KERN_SUCCESS) {
            [log appendLevel:@"INFO"
                          key:@"MACH_VM_7FFE0000_MUTATION"
                        value:@"FIXED_MAP_SUCCEEDED_AND_RELEASED"];
        } else {
            [log appendLevel:@"ERROR"
                          key:@"MACH_VM_7FFE0000_MUTATION"
                        value:@"FIXED_MAP_SUCCEEDED_RELEASE_FAILED"];
        }
    }
}
